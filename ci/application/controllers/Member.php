...
class Member extends W_Controller {
    ...
    public function __construct() {
        ...
        ,   'FACEBOOK_CALLBACK_URL' => urlencode($this->config->item('site_domain_https').'/member/facebook_callback?r_url='. urlencode($this->input->get('r_url')?$this->input->get('r_url'):'/'))
        ...
        ,   'GOOGLE_REQUEST_URL'     => "https://kauth.kakao.com/oauth/authorize?client_id=".KAKAO_CLIENT_KEY."&redirect_uri=".urlencode($this->config->item('default_http').'/member/kakaoSyncCall')."&response_type=code&state=".urlencode($this->input->get('r_url'))

        ,   'join_path'             => $join_path
        ,   'rUrl'                  => $this->input->get('r_url')
        ,   'rUrl_kakao'            => $this->input->get('state') //kakao 에서 콜백 파라메터를 전달할수 없어 임시로 state로 처리
        );
    ...
    private function join_proc(){
        ...
        if( empty($member_row) ) { //없으면 => 가입
            ...
            log_message('p',json_encode_no_slashes($query_data));
