...
class Builder
{
     ...
     * @var string
     */
    const VERSION = '1.3.4'; // Application's version, it's not document version.
    ...
    protected $_version_dir;
    /**
     * Constructor
     *
     * @param array $st_classes
     */
    public function __construct(array $st_classes, $s_output_dir, $title = 'php-apidoc', $s_output_file = 'index.html', $template_path = null, $_version_dir, $logo)
    {
        ...
        $this->_version_dir = $_version_dir;

        $this->doc_version = array_pop(explode('/', $s_output_dir));
    ...
    protected function saveTemplate($data, $file)
    {
        ...
        $tr = array(
            ...
            '{{ last_writer }}' => $_SESSION['session_m_num'],
            '{{ doc_version }}' => $this->doc_version,
            '{{ version }}' => static::VERSION
        );

        // 버전 셀렉트 박스
        $tmp_version_dir = '<select id="version">';
        foreach($this->_version_dir as $key => $val) {

            if ($this->doc_version == $val) {
                $selected = 'selected';
            }
            $tmp_version_dir .= '<option value="' . $val . '"' . $selected . '>' . $val . '</opiton>';
        }
        $tmp_version_dir .= '</select>';

        $tr['{{ version_dir }}'] = $tmp_version_dir;
        ...
        foreach ($st_annotations as $class => $methods) {
            foreach ($methods as $name => $docs) {
                ...
                $tr = array(
                    ...
                    '{{ link }}'                    => $docs['ApiLink'][0]['description'],
    ...
    protected function generateSandboxForm($st_params, $counter)
    {
        ...
        if (isset($st_params['ApiHeaders']) && is_array($st_params['ApiHeaders']))
        {
            ...
            {
                if ($header['name'] == 'm_key') {
                    $value = $_SESSION['session_m_key'];
                } else if ($header['name'] == 'm_num') {
                    $value = $_SESSION['session_m_num'];
                }

                $headers[] = strtr(static::$sandboxFormInputTpl, array('{{ name }}' => $header['name'], '{{ value }}' => $header['value']));
            }
        }
