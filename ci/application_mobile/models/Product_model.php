 ...
 * 상품 관련 모델
 */
class Product_model extends M_Model {
...
//            if(is_array($query_array['where']['ctgr']) == true){
//
//                $ctgr_arr = explode(',',$query_array['where']['ctgr']);
//                $ctgr_query = array();
//
//                foreach ($ctgr_arr as $v) {
//                    $ctgr_query[] .= " p_cate1 = '{$v}' ";
//                }
//                $where_query .= " AND ( ". implode(' OR ', $ctgr_query) . " ) " ;
//
//            }else {
//                $where_query .= " AND p_cate1 = '{$query_array['where']['ctgr']}' " ;
//            }
        ...
        if($query_array['main_best'] == 'Y'){
            ...
            $order_query .= " ORDER BY p_order_count_week DESC , p_termlimit_datetime1 DESC ";
    ...
    public function get_product_row($arrayParams) {
        ...
        if( empty($arrayParams['p_num']) == true && empty($arrayParams['p_order_code']) == true ) {
            return false;
        }
        ...
        if(empty($arrayParams['p_order_code']) == false) $addQueryString .= " AND p_order_code = '{$arrayParams['p_order_code']}' ";

        if(empty($arrayParams['p_display_state']) == true) {
            $addQueryString .= " AND p_display_state = 'Y' ";
        }else{
            $addQueryString .= " AND p_display_state = '{$where_array['p_display_state']}' ";
        }
        if(empty($arrayParams['p_sale_state']) == true) {
            $addQueryString .= " AND p_sale_state = 'Y' ";
        }else{
            $addQueryString .= " AND p_sale_state = '{$where_array['p_sale_state']}' ";
        }
