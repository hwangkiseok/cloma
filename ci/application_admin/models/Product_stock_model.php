 ...
 * 상품 관련 모델
 */
class Product_stock_model extends A_Model {
    ...
    public function get_product_list($query_array=array(), $start="", $end="", $is_count=false, $DB=false) {
        ...
        if( isset($query_array['where']['proc_yn']) && !empty($query_array['where']['proc_yn']) ) {
            $where_query .= "and sct.proc_yn = '" . $this->db->escape_str($query_array['where']['proc_yn']) . "' ";
        }

        if( isset($query_array['where']['issue_yn']) && !empty($query_array['where']['issue_yn']) ) {
            $where_query .= "and sct.issue_yn = '" . $this->db->escape_str($query_array['where']['issue_yn']) . "' ";
        }
        ...
        if( isset($query_array['orderby']) && !empty($query_array['orderby']) ) {
            ...
            $order_query = "order by p_date desc, sct.p_num desc ";
        }
        ...
        if( $is_count === TRUE ) {
            $query = "SELECT 
                      ...
                      INNER JOIN `product_tb` pt ON sct.p_order_code = pt.p_order_code
        ...
        //데이터 추출
            ...
            $query = "SELECT 
                        sct.* 
                      ,	pt.*
                      , ( IF ( pt.p_termlimit_datetime1 = '' , pt.p_regdatetime, pt.p_termlimit_datetime1 )) AS p_date
                      ,	spt.option_info
                      , CASE WHEN sct.mod_id = 9999999 THEN '시스템변경' ELSE (SELECT au_name FROM adminuser_tb WHERE au_num = sct.mod_id ) END AS au_name
