...
/**
 * 페이지네이션 수정 : 처음, 이전, 다음, 마지막 링크를 항상 출력하도록 수정
 */
class A_Pagination extends CI_Pagination {
    ...
    public function create_links()
    {
        ...
        if ($this->page_query_string === TRUE)
        {
            $this->cur_page = $this->CI->input->post_get($this->query_string_segment);
        }
        else
        {
        ...
        if( !preg_match("/page=1/", $first_url) ) {
            if( preg_match("/\?/", $first_url) ) {
                $first_url .= "&page=1";
            }
            else {
                $first_url .= "?page=1";
