            ...
            <div class="box">
                <div class="box-content">
                    <form name="pop_update_form" id="pop_update_form" method="post" class="form-horizontal" role="form" action="/review/update_proc">
                        ...
                        <input type="hidden" name="pm_uid" value="" />
                            ...
                            <div class="col-sm-10">
                                <div id="field_re_display_state">
                                    <div id="point_display_before">
                                        <?php echo get_input_radio("points_state", $this->config->item("point_display_state"), "", $this->config->item("point_display_state_text_color")); ?>
                                        <input type="button" onclick="reg_point('<?php echo $req['re_num']; ?>');" value="적용">
                                    </div>
...
<script>
    ...
    function reg_point(re_num) {
        ...
        if ($('input[name="points_state"]:checked').length < 1) {
            alert('적용할 적립금을 선택해주세요.');
            return false;
        }

        var cf = confirm('선택하신 적립금을 적용하시겠습니까?');
        if(!cf) return false;

        $.ajax({
            url: '/point/insertReviewPointMember/',
            data: { point_state : $('input[name="points_state"]:checked').val() , re_num : $('input[name="re_num"]').val() },
            type: 'post',
            dataType: 'json',
            cache: false,
            success: function (result) {
    ...
    // 적립금 삭제
    function delete_point(id, m_key, re_num) {
        ...
        $.ajax({
            ...
            data: { id : id, m_key : m_key, re_num : re_num },
            type: 'post',
            dataType: 'json',
            cache: false,
            success: function (result) {
    ...
    function chk_pointmember(re_num) {
        $.ajax({
            url: '/point/chkPointMembers/',
            data: { re_num : $('input[name="re_num"]').val() },
            type: 'post',
            dataType: 'json',
            cache: false,
            success: function (result) {
                if(result.given_flag == 'Y') {
                    $('input[name="pm_uid"]').val(result.id);
                    $("#point_display_before").hide();
                    $("#point_display_after").html(result.given_name + ' ' +

                        "<input type='button' onclick=\"delete_point('" + result.id + "', '" + result.m_key + "', '<?php echo $req['re_num']; ?>');\" value='삭제'>"
                    );
