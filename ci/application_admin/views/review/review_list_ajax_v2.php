...
<div class="table-responsive">
    ...
    <table class="table table-hover table-bordered <?php if( $req['rctly'] != "Y" ) { ?>dataTable<?php } ?>">
        ...
        <tr role="row" class="active">
            ...
            <th style="width:75px;" class="<?php echo $sort_array['re_display_state'][1];?>" onclick="form_submit('sort_field=re_display_state&sort_type=<?php echo $sort_array['re_display_state'][0]; ?>');">CS처리</th>
            <th style="width:75px;">적립금</th>
            <?if($req['winner_mode'] == 'Y'){?><th style="width:75px;">포토리뷰 당첨</th><?}?>
        ...
        <!-- 베스트 리뷰 -->
        <?php if($best_review_list): ?>

            <?php foreach($best_review_list as $row) {
                $aImg = json_decode($row->re_img);
            ?>
            <tr role="row" class="cmt-item" data-flag_set="<?=$row->re_blind.'||'.$row->re_display_state.'||'.$row->re_recommend?>" style="background-color:#FFEAEA">
                <td>
                    <input type="checkbox" id="re_list_<?php echo $row->re_num; ?>" name="re_list" value="<?php echo $row->re_num;; ?>" img-cnt="<?php echo count($aImg); ?>" reward_yn="<?php echo $row->re_reward; ?>" admin_yn="<?php echo $row->re_admin; ?>" <?php echo ($row->re_reward == 'Y' || $row->re_admin == 'Y') ? 'disabled' : ''; ?> ">
                </td>
                <td style="font-weight: bold; color: #FF0000">[<?php echo $row->re_sort; ?>]</td>
                <td>
                    ...
                    <br>
                    <button type="button" onclick="moveUp(this, '<?php echo $row->re_num; ?>', <?php echo $row->re_table_num; ?>)">▲</button>
                    <button type="button" onclick="moveDown(this, '<?php echo $row->re_num; ?>', <?php echo $row->re_table_num; ?>)">▼</button>
                </td>
                <td class="comm" style="text-align:left;<?php if($row->re_admin == 'Y') { ?>color:#0000ff;<?php }//endif; ?>">
                    <?if(count($aImg) > 0){?>
                        <? foreach ($aImg as $key=>$val) {?>
                            <div class="thumbnail" style="display: inline-block;">
                                <a href="#none" onclick="new_win_open('<?=$val?>', 'img_pop', 800, 600);" style="display: inline-block;">
                                    <img src="<?=$val?>" width="123" alt="">
                                </a>
                                <?if($req['winner_mode'] == 'Y'){?>
                                    <div class="caption" style="padding: 0">
                                        <button class="btn btn-info btn-xs" onclick="copy_clipboard_n('<?=$this->config->item('default_http');?>/product/getReviewPop/?seq=<?=$row->re_num; ?>&img_select=<?=$key?>');">리뷰상세</button>
                                        <button class="btn btn-warning btn-xs" onclick="copy_clipboard_n('<?=$row->re_table_num?>');">상품번호</button>
                                    </div>
                                <?}?>
                            </div>
                        <?}?>
                ...
                <td id="point_givent_yn_<?php echo $row->re_num; ?>">
                    <?php echo $row->re_admin == 'Y' ? '-' : ($row->re_reward_type == 'A' ? "텍스트" : ($row->re_reward_type == 'B' ? "포토" : ($row->re_reward_type == 'C' ? "지급안함" : "미지급"))); ?>
                </td>

                <?if($req['winner_mode'] == 'Y'){?>
                    <td>
                        <?if($row->re_winner_yn == 'Y'){?>
                            <button class="btn btn-success btn-xs" onclick="setReviewFlag('re_winner_yn','N','<?=$row->re_num?>',this);"><?=view_date_format($row->re_winner_date,11)?><br>당첨</button>
                        <?}else{?>
                            <button class="btn btn-danger btn-xs" onclick="setReviewFlag('re_winner_yn','Y','<?=$row->re_num?>',this);">비당첨</button>
                        <?}?>
                    </td>
                <?}?>
        ...
        foreach($review_list as $row) {
            ...
            <tr role="row" class="cmt-item" data-flag_set="<?=$row->re_blind.'||'.$row->re_display_state.'||'.$row->re_recommend?>" >
                ...
                <td class="comm" style="text-align:left;<?php if($row->re_admin == 'Y') { ?>color:#0000ff;<?php }//endif; ?>">
                    ...
                    <?php if(!empty($row->re_blind_memo)): ?>
                    <br><br>
                    <div style="background-color: #eaeaea; padding: 5px;">
                        <b>[ 참고메모 ]</b><br>
                        <?php echo nl2br($row->re_blind_memo); ?>
                    </div>
                    <?php endif; ?>
                </td>
                <td>
                ...
                <!--
                전체공개 삭제 후, CS필요/불필요 컬럼추가 dhkim 20190710
                <td>
                    <?if($row->re_display_state == 'Y'){?>
                        <button class="btn btn-success btn-xs" onclick="setReviewFlag('re_display_state','N','<?=$row->re_num?>',this);" >공개</button>
                    <?}else{?>
                ...
                -->
                <td>
                    <?if($row->re_cs_help_yn == 'Y'){?>
                        <button class="btn btn-success btn-xs" onclick="setReviewFlag('re_cs_help_yn','N','<?=$row->re_num?>',this);" >필요</button>
                    <?}else{?>
                        <button class="btn btn-danger btn-xs" onclick="setReviewFlag('re_cs_help_yn','Y','<?=$row->re_num?>',this);" >불필요</button>
                    <?}?>
                </td>
                <td id="point_givent_yn_<?php echo $row->re_num; ?>">
                    <?php echo $row->re_admin == 'Y' ? '-' : ($row->re_reward_type == 'A' ? "텍스트" : ($row->re_reward_type == 'B' ? "포토" : ($row->re_reward_type == 'C' ? "지급안함" : "미지급"))); ?>
...
<script>
        ...
        $("#all_chk").on('click', function() {
            ...
            if($(this).prop("checked") == true) {
                ...
                $("input[name='re_list']").each(function() {
                    ...
                    if ($(this).attr('reward_yn') == 'N' && $(this).attr('admin_yn') == 'N') {
    ...
    function batch_apply_point() {
        ...
        var reward_type = $('input[name="sel_reward_type"]:checked').val(); // A: 텍스트, B: 포토
        ...
        if(reward_type == 'A' || reward_type == 'B') {
            ...
            var res = confirm('선택한 리뷰에 적립금 지급안함으로 적용하시겠습니까?');
        }

        if(res) {
            $.ajax({
                url : '/point/batchReviewApply',
                data : { re_list : lists, re_type : reward_type, status : status },
                type : 'post',
                dataType : 'json',
    ...
    function moveUp(el, re_num, re_table_num){

        var $tr = $(el).parent().parent(); // 클릭한 버튼이 속한 tr 요소
        var now_index = parseInt($tr.index());

        if(now_index <= 0) {
            alert('선택하신 인덱스를 더 올릴 수 없습니다.');
            return false;
        } else {
            $tr.prev().before($tr); // 현재 tr 의 이전 tr 앞에 선택한 tr 넣기

            $.ajax({
                url: '/review/setBestReviewSort',
                data: {re_num: re_num, re_table_num: re_table_num, now_index: now_index, type: 'up'},
                type: 'post',
                dataType: 'json',
                success: function (result) {
                    $('#search_form').submit();
    ...
    function moveDown(el, re_num, re_table_num){

        var limit_cnt = <?php echo $best_review_list_cnt; ?>;
        var $tr = $(el).parent().parent(); // 클릭한 버튼이 속한 tr 요소
        var now_index = parseInt($tr.next().index()) + 1;

        if(limit_cnt < now_index) {
            alert('선택하신 인덱스를 더 내릴 수 없습니다.');
            return false;
        } else {
            ...
            $.ajax({
                url : '/review/setBestReviewSort',
                data : { re_num : re_num, re_table_num : re_table_num, now_index : now_index, type : 'down' },
                type : 'post',
                dataType : 'json',
                success : function(result) {
                    $('#search_form').submit();
