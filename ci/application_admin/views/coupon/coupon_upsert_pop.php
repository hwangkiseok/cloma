            ...
            <div class="box">
                <div class="box-content">
                    <form name="coupon_frm" id="coupon_frm" method="post" class="form-horizontal" role="form" action="<?php echo $this->page_link->update_proc; ?>">
                        ...
                        <div class="form-group form-group-sm">
                            <label class="col-sm-2 control-label">쿠폰 <span class="txt-danger">*</span></label>
                            <div class="col-sm-10 form-inline">
                                <div id="field_coupon_info">
                                    <select name="coupon_select" class="form-control" style="width: 100%">
                                        <option value="">선택해주세요!</option>
                                        <? foreach ($aCouponLists as $row) {?>
                                            <option value="<?=$row['cpn_code']?>::<?=$row['cpn_uid']?>::<?=$row['cpn_name']?>::<?=$row['cpn_issue_type']?>"><?=$row['cpn_name']?> [ <?=number_format($row['cpn_price'])?>할인 / 최소 <?=number_format($row['cpn_min_orderprice'])?>원 ] </option>
                                        <? } ?>
                                    </select>
                                </div>
                                <div id="field_coupon_select_lists">
                                    <? foreach ($aSelectedCouponLists as $row) {?>
                                        <p class="alert alert-warning coupon-info-alert"><input type="hidden" name="coupon_info[]" value="<?=$row['cpn_code']?>::<?=$row['cpn_uid']?>::<?=$row['cpn_name']?>::<?=$row['cpn_issue_type']?>" /><span><?=$row['cpn_name']?> [ <?=number_format($row['cpn_price'])?>할인 / 최소 <?=number_format($row['cpn_min_orderprice'])?>원 ]</span><button class="pull-right btn btn-danger btn-xs coupon-row-del">삭제</button></p>
                                    <?}?>
                                </div>
                            </div>
                        </div>
