...
<div class="container-fluid">
    <div class="row">
        <h4 class="page-header">카카오스토리 > 상품관리</h4>
    </div>
        ...
        <form name="search_form" id="search_form" method="post" action="<?php echo $this->page_link->list_ajax; ?>" class="form-horizontal">
                    ...
                    <div class="input-group">
                        <span class="input-group-btn" style="width:auto">
                            <select id="kfd" name="kfd" class="form-control" style="border-bottom-left-radius:3px;border-top-left-radius:3px;">
                                ...
                                <option value="ksp_title">제목</option>
                                <option value="ksp_content">내용</option>
                            </select>
                        </span>
            ...
            <div class="form-group form-group-sm">
                ...
                <div class="col-sm-10 mgt3" id="product_state_checkbox">
                    ...
                    <?php
                    foreach( $this->config->item('kstory_product_display_state') as $key => $text ) {
            ...
            <div class="form-group form-group-sm text-center">
                ...
                <a href="#none" class="btn btn-info btn-sm mgl10" style="width:100px;" onclick="kstory_product_insert_pop();">등록</a>
            </div>
        </form>
    </div>
...
<script>
     ...
     * @param fd
     */
    function kstory_product_update_toggle(ksp_num, fd) {
        if( empty(ksp_num) || empty(fd) ) {
            return false;
        }
        ...
        $.ajax({
            url : '/kstory_product/update_toggle',
            data : {ksp_num:ksp_num, fd:fd},
            type : 'post',
            dataType : 'json',
     ...
     * 카카오스토리 상품 등록 팝업
     */
    function kstory_product_insert_pop() {
        var container = $('<div>');
        $(container).load('<?php echo $this->page_link->insert_pop; ?>/?' + $('#search_form').serialize());

        modalPop.createPop('카카오스토리 상품 등록', container);
    ...
    }//end of kstory_product_insert_pop()

    /**
     * 카카오스토리 상품 수정 팝업
     */
    function kstory_product_update_pop(ksp_num) {
        var container = $('<div>');
        $(container).load('<?php echo $this->page_link->update_pop; ?>/?ksp_num=' + ksp_num + '&' + $('#search_form').serialize());

        modalPop.createPop('카카오스토리 상품 수정', container);
    ...
    }//end of kstory_product_update_pop()

    /**
     * 카카오스토리 상품 삭제
     */
    function kstory_product_delete(ksp_num) {
        if( empty(ksp_num) ) {
            return false;
        }
        if( !confirm('삭제하시겠습니까?') ) {
    ...
    }//end of kstory_product_delete()

    /**
     * 카카오스토리 상품 단축 URL 추출
     */
    function get_kstory_product_shorten_url(ksp_num) {
        if( empty(ksp_num) ) {
            return false;
        }
        ...
        $.ajax({
            url : '/kstory_product/shorten_url',
            data : {ksp_num:ksp_num},
            type : 'post',
            dataType : 'json',
            async: false,
    ...
    }//end of get_kstory_product_shorten_url()
    ...
    $(function () {
        ...
        $(document).on('click', '#all_list_check', function(){
            $('[name="ksp_num[]"]').prop('checked', $(this).prop('checked'));
        });
