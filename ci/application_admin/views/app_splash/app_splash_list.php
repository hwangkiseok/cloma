<div class="container-fluid">
    <div class="row">
        <h4 class="page-header">APP 스플래시관리 > 목록</h4>
    </div>
    ...
    <div class="row">
        <form name="search_form" id="search_form" method="post" action="<?php echo $this->page_link->list_ajax; ?>" class="form-horizontal">
            ...
            <div class="form-group form-group-sm text-center">
                <button type="button" class="btn btn-info btn-sm mgl10" onclick="app_splash_insert_pop();" style="width:100px;">등록</button>
            </div>
        </form>
    </div>
    ...
    <div class="row">
        ...
        <div id="app_splash_list" style="position:relative;"></div>
    </div>
</div>
...
<script>
     ...
     * APP 스플래시 수정
     */
    function app_splash_update_pop(aps_num) {
        var container = $('<div>');
        $(container).load('<?php echo $this->page_link->update_pop; ?>/?aps_num=' + aps_num);

        modalPop.createPop('APP 스플래시 수정', container);
     ...
     * APP 스플래시 삭제
     */
    function app_splash_delete(aps_num) {
        if( !confirm('삭제하시겠습니까?') ) {
            return false;
        }
    ...
    }//end of app_splash_delete()

    /**
     * 사용함/사용안함
     * @param aps_num
     */
    function app_splash_usestate_toggle(aps_num) {
        if( empty(aps_num) ) {
            return false;
        }
        ...
        $.ajax({
            url : '/app_splash/usestate_toggle',
            data : {aps_num:aps_num},
            type : 'post',
            dataType : 'json',
            success : function (result) {
    ...
    }//end of app_splash_display_toggle()
    ...
    $(function () {
        ...
        $('#search_form').ajaxForm({
            ...
            beforeSubmit: function(formData, jqForm, options) {
                loadingBar.show($('#app_splash_list'));
            },
            success: function(result) {
                $('#app_splash_list').html(result);
            },
            complete: function() {
                loadingBar.hide();
            }
        ...
        //ajax page
        $(document).on('click', '#app_splash_list .pagination.ajax a', function(e){
            e.preventDefault();
