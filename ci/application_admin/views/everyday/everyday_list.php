<div class="container-fluid">
    <div class="row">
        <h4 class="page-header">매일응모 > 목록</h4>
    </div>
    ...
    <div class="row">
        <form name="search_form" id="search_form" method="post" action="<?php echo $this->page_link->list_ajax; ?>" class="form-horizontal">
            ...
            <div class="form-group form-group-sm">
                <label class="col-sm-2 control-label" for="usestate">진행여부</label>
                <div class="col-sm-10">
                    <select id="usestate" name="usestate" class="form-control" style="width:auto;">
                        <?php echo get_select_option("* 전체 *", $this->config->item("everyday_usestate"), $req['usestate']); ?>
                    </select>
                </div>
            </div>
            <div class="form-group form-group-sm">
                <label class="col-sm-2 control-label" for="displaystate">노출여부</label>
                <div class="col-sm-10">
                    <select id="displaystate" name="displaystate" class="form-control" style="width:auto;">
                        <?php echo get_select_option("* 전체 *", $this->config->item("everyday_displaystate"), $req['displaystate']); ?>
                    </select>
                </div>
            </div>
            <div class="form-group form-group-sm">
            ...
            <div class="form-group form-group-sm text-center">
                ...
                <button type="button" class="btn btn-info btn-sm mgl10" onclick="everyday_insert_pop();" style="width:100px;">등록</button>
...
<script>
     ...
     * 매일응모 수정 팝업
     */
    function everyday_update_pop(ed_num) {
        var container = $('<div>');
        $(container).load('<?php echo $this->page_link->update_pop; ?>/?ed_num=' + ed_num);

        modalPop.createPop('매일응모 수정', container);
        modalPop.createButton('수정', 'btn btn-primary btn-sm', function(){
            $('#pop_update_form').submit();
        });
        modalPop.createCloseButton('취소', 'btn btn-default btn-sm');
        modalPop.show();
    }//end of everyday_update_pop()

    /**
     * 매일응모 삭제
     */
    function everyday_delete(ed_num) {
        if( !confirm('삭제하시겠습니까?') ) {
            return false;
        }
     ...
     * @param fd
     */
    function everyday_update_toggle(ed_num, fd) {
        if( empty(ed_num) || empty(fd) ) {
            return false;
        }
        ...
        $.ajax({
            url : '/everyday/update_toggle',
            data : {ed_num:ed_num, fd:fd},
            type : 'post',
            dataType : 'json',
            success : function (result) {
