...
var stock_on = 'on';
var shipping_text = '';
var flag_s_price = 'N'; //2차판매가 여부
var flag_t_price = 'N'; //3차판매가 여부
var pdt_add_option_type = 1; //1:기본 / 2:1+1 / 3:1+1+1
// if( !$.browser.android ) {
//     modal_yn = true;
// }
...
function chk_option_stock(options,depth){

    var view_chk = false;

    if(depth == 1) { //1차 옵션
        $.each(options, function (k, r) {
            if (r.stock > 1 && r.stock <= 10) view_chk = true;
        });
    }else if(depth == 'no'){ //
        view_chk = true;
    }else{ // 2/3차 옵션
        $.each(options,function(k,r){
            $.each(r,function(kk,rr){ if(rr.stock > 1 && rr.stock <= 10) view_chk = true; });
        });
    }

    if(zsDebug == '1'){
        if(view_chk == true){ //매진 임박 스크립트 추가
            //console.log('매진임박');
            $('.soldout_banner').show();
        }
    }
    ...
    $.ajax({
        ...
        success : function (result) {
            if( !empty(result) ) {
                ...
                if(info.sendcompany == 'CJ대한통운'){
                    //$('.detail_wrap .detail_comm').prepend('<img src="http://09sns.co.kr/upfiles/product_desc/2575181700" />');
                }
                ...
                if(info.sum_delivery > 0 && info.sum_delivery_yn == 'Y'){
                    $('.add-item .shipping_info').next().append('('+ info.sum_delivery+'개 단위 합배송, 초과시 배송비추가)');
                }

                $('.add-item .shipping_info').html(shipping_text);
                $('.add-item .shipping_info').show();

                pdt_add_option_type = info.add_option_type;

                pdt_option_type = info.option_type;
                empty_stock_cnt = info.empty_stock_cnt;

                flag_t_price = info.price_third_yn;
                flag_s_price = info.price_second_yn;

                pdt_option_data = options;
                pdt_other_data = others;

                if(pdt_option_type == '3depth') {
                    chk_option_stock(options[3],2);
                }else if(pdt_option_type == '2depth') {
                    chk_option_stock(options[2],2);
                }else if(pdt_option_type == '1depth') {
                    chk_option_stock(options[1],1);
                }else if(pdt_option_type == 'nooption') {
                    if(info.stock >= 1 && info.stock <= 10) chk_option_stock(options,'no');
                }
                ...
                if( pdt_option_type == 'nooption' || empty(options) ) {
                    ...
                    //console.log(nv_shop ,appCheck() ,info.price_third_yn,info.price_third);

                    if( nv_shop == 'Y' && appCheck() == false && !empty(info.price_third_yn) && info.price_third_yn == "Y" && !empty(info.price_third) ) { //판매가 설정(3차 판매가 체크)
                        price = info.price_third;
                    }else if( appCheck() == false && !empty(info.price_second_yn) && info.price_second_yn == "Y" && !empty(info.price_second) ) { //판매가 설정(2차 판매가 체크)
                    ...
                    if(info.stock < 21){ // 재고수량이 21개 미만인 경우 아래 태그 삽입
                        html += '<span class="deadline2">매진임박 (<em>'+info.stock+'개</em>)</span>';
                    }
                    ...
                    var addNotiHtml = '';
                    if(info.shopping_pay_free > 0){
                        //<div class="op_deliverNoti">15,000원 이상 구매시 무료배송! 배송비를 절약하세요. </div>
                        addNotiHtml = '<div class="op_deliverNoti" style="font-size:13px; font-weight:400; padding:5px; background:#eff7f5; color:#006b4b; text-align:center">'+number_format(info.shopping_pay_free)+'원 이상 구매시 무료배송! 배송비를 절약하세요.</div>';
                    }
                    html += addNotiHtml;
                ...
                //옵션이 있을때
                    ...
                    if(info.add_option_type == 2) {
                        html += '<input type="hidden" name="pdt_option_add_1" value="" />';
                        html += '<input type="hidden" name="pdt_option_add_2" value="" />';

                        html += '<div class="select_opt" role="button" data-no="1">옵션을 선택해 주세요.1</div>';
                        html += '<div class="select_opt" role="button" data-no="2">옵션을 선택해 주세요.2</div>';
                    }else if(info.add_option_type == 3){
                        html += '<input type="hidden" name="pdt_option_add_1" value="" />';
                        html += '<input type="hidden" name="pdt_option_add_2" value="" />';
                        html += '<input type="hidden" name="pdt_option_add_3" value="" />';

                        html += '<div class="select_opt" role="button" data-no="1">옵션을 선택해 주세요.1</div>';
                        html += '<div class="select_opt" role="button" data-no="2">옵션을 선택해 주세요.2</div>';
                        html += '<div class="select_opt" role="button" data-no="3">옵션을 선택해 주세요.3</div>';
                    }else{
...
function get_product_option_html_new(depth,opt,child_opt){
    ...
    if(depth == 1) { //1차 옵션
        ...
        $(opt).each(function (index, row) {
            ...
            var deadline_html   = '';
            var deadline_class  = '';
            var sale_price = row.price;
            //판매가 설정(2차 판매가 체크)

            //console.log(row);

            if( nv_shop == 'Y' && appCheck() == false && flag_t_price == "Y" && !empty(row.third_price) && row.third_price > 0) {
                sale_price = row.third_price;
            }else if( appCheck() == false && flag_s_price == "Y" && !empty(row.second_price) && row.second_price > 0) {
                sale_price = row.second_price;
            }
            ...
            html += '<li class="' + deadline_class + '" data-name="' + row.name + '" data-uid="' + row.uid + '" data-depth="' + depth + '" data-stock="' + row.stock + '" data-price="' + sale_price + '" onclick="op_select_click_new(this);">';
            html += '    <a>';
            ...
            html += deadline_html;
            html += '        <span class="prc"><b>'+number_format(sale_price)+'</b>원</span>';
            html += '    </a>';
            html += '</li>';
...
function set_pdt_option_2_new() {
    ...
    $.each(pdt_option_data[2][uid], function(index, row){
        ...
        if( nv_shop == 'Y' && appCheck() == false && flag_t_price == "Y" && !empty(row.third_price) && row.third_price > 0) { //판매가 설정(3차 판매가 체크)
            sale_price = row.third_price;
        }else if( appCheck() == false && flag_s_price == "Y" && !empty(row.second_price) && row.second_price > 0) { //판매가 설정(2차 판매가 체크)
            sale_price = row.second_price;
        }
        ...
        html += '<li class="' + deadline_class + '" data-name="' + row.name + '" data-uid="' + row.uid + '" data-depth="2" data-stock="' + row.stock + '" data-price="' + sale_price + '" onclick="op_select_click_new(this);">';
        html += '    <a>';
...
function set_pdt_option_3_new() {
    ...
    $.each(pdt_option_data[3][uid], function(index, row){
        ...
        html += '<li class="' + deadline_class + '" data-name="' + row.name + '" data-uid="' + row.uid + '" data-depth="3" data-stock="' + row.stock + '" data-price="' + sale_price + '" onclick="op_select_click_new(this);">';
        html += '    <a>';
 ...
 * 1+1 옵션 최종 선택시
 */
function selected_add_option_add() {
    ...
    $('.select_opt[data-no="'+select_option_no+'"]').html(name);
    $('input[name="pdt_option_add_'+select_option_no+'"]').val(uid);



    var temp_val = '';
    var complete_select = false;
    if(pdt_add_option_type == 2){

        if( empty($('input[name="pdt_option_add_1"]').val()) == false && empty($('input[name="pdt_option_add_2"]').val()) == false ){
            complete_select = true;
            if($('input[name="pdt_option_add_1"]').val() > $('input[name="pdt_option_add_2"]').val()){
                temp_val = $('input[name="pdt_option_add_2"]').val()+'||'+$('input[name="pdt_option_add_1"]').val();
            }else{
                temp_val = $('input[name="pdt_option_add_1"]').val()+'||'+$('input[name="pdt_option_add_2"]').val();
            }

        }

    }else if (pdt_add_option_type == 3){

        if( empty($('input[name="pdt_option_add_1"]').val()) == false && empty($('input[name="pdt_option_add_2"]').val()) == false && empty($('input[name="pdt_option_add_3"]').val()) == false ){
            complete_select = true;

            if( $('input[name="pdt_option_add_1"]').val() > $('input[name="pdt_option_add_2"]').val() && $('input[name="pdt_option_add_1"]').val() > $('input[name="pdt_option_add_3"]').val() ){
                if( $('input[name="pdt_option_add_2"]').val() > $('input[name="pdt_option_add_3"]').val()){
                    temp_val = $('input[name="pdt_option_add_3"]').val()+'||'+$('input[name="pdt_option_add_2"]').val()+'||'+$('input[name="pdt_option_add_1"]').val();
                }else{
                    temp_val = $('input[name="pdt_option_add_2"]').val()+'||'+$('input[name="pdt_option_add_3"]').val()+'||'+$('input[name="pdt_option_add_1"]').val();
                }

            }else if( $('input[name="pdt_option_add_2"]').val() > $('input[name="pdt_option_add_1"]').val() && $('input[name="pdt_option_add_2"]').val() > $('input[name="pdt_option_add_3"]').val() ){
                if( $('input[name="pdt_option_add_1"]').val() > $('input[name="pdt_option_add_3"]').val()){
                    temp_val = $('input[name="pdt_option_add_3"]').val()+'||'+$('input[name="pdt_option_add_1"]').val()+'||'+$('input[name="pdt_option_add_2"]').val();
                }else{
                    temp_val = $('input[name="pdt_option_add_1"]').val()+'||'+$('input[name="pdt_option_add_3"]').val()+'||'+$('input[name="pdt_option_add_2"]').val();
                }

            //}else if($('input[name="pdt_option_add_3"]').val() > $('input[name="pdt_option_add_2"]').val() && $('input[name="pdt_option_add_3"]').val() > $('input[name="pdt_option_add_1"]').val()){
            }else{

                if( $('input[name="pdt_option_add_1"]').val() > $('input[name="pdt_option_add_2"]').val()){
                    temp_val = $('input[name="pdt_option_add_2"]').val()+'||'+$('input[name="pdt_option_add_1"]').val()+'||'+$('input[name="pdt_option_add_3"]').val();
                }else{
                    temp_val = $('input[name="pdt_option_add_1"]').val()+'||'+$('input[name="pdt_option_add_2"]').val()+'||'+$('input[name="pdt_option_add_3"]').val();
                }
    ...
    if(complete_select == true){
        ...
        var _final_select_option = new Object();
        _final_select_option = pdt_option_data.real_result[temp_val];

        // console.log(_final_select_option.po_uid);
        // console.log('1',$('input[name="pdt_option_add_1"]').val());
        // console.log('2',$('input[name="pdt_option_add_2"]').val());
        // console.log('3',$('input[name="pdt_option_add_3"]').val());
        console.log(sel_obj);

        if(pdt_add_option_type == 2){
            $('.select_opt[data-no="1"]').html('옵션을 선택해주세요.');
            $('.select_opt[data-no="2"]').html('옵션을 선택해주세요.');
            $('input[name="pdt_option_add_1"]').val('');
            $('input[name="pdt_option_add_2"]').val('');
        }else if(pdt_add_option_type == 3){
            $('.select_opt[data-no="1"]').html('옵션을 선택해주세요.');
            $('.select_opt[data-no="2"]').html('옵션을 선택해주세요.');
            $('.select_opt[data-no="3"]').html('옵션을 선택해주세요.');
            $('input[name="pdt_option_add_1"]').val('');
            $('input[name="pdt_option_add_2"]').val('');
            $('input[name="pdt_option_add_3"]').val('');
        }
        ...
        //이미 선택된 옵션이면
        if( $('.op_selected_item[data-uid="' + _final_select_option.po_uid + '"]').length > 0 ) {
            update_op_cnt(_final_select_option.po_uid, '+', 1);
            $('.op_layer').hide();
            return false;
        }

        selected_option_add_html(_final_select_option.po_uid, _final_select_option.po_poptionname , price, stock, 'N');
        update_total_price();
        set_op_data();
    }
...
function update_total_price() {
    ...
    if($('[name="shopping_pay_free"]').val() <= total_price){
        $('.op_deliverNoti').hide();
    }else{
        $('.op_deliverNoti').show();
    }
    set_pdt_option_wrap_position('Y');
 ...
 * 옵션창 위치 설정
 */
function set_pdt_option_wrap_position(noAni) {
    /*
    ...
    */
    ...
    if( $('.op_deliverNoti').length < 1){
        top  = top + $('.op_deliverNoti').outerHeight(true);
    }
...
function op_add_select_click_new(obj){
...
function op_select_click_new(obj) {
    ...
    if(pdt_add_option_type == '2' || pdt_add_option_type == '3'){
        op_add_select_click_new(obj);
        return ;
    }
